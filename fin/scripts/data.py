...
def preprocess(system):
    fcc_svn = pd.read_csv(f'{system}-fcc.csv')
    hcp_svn = pd.read_csv(f'{system}-hcp.csv')
    fcc_dft = pd.read_csv(f'{system}.fcc.out')
    hcp_dft = pd.read_csv(f'{system}.hcp.out')
    dft = pd.DataFrame()
    dft['ratio'] = fcc_dft['ratio']
    dft['vol-fcc'] = fcc_dft['volume']
    dft['energy-fcc'] = fcc_dft['energy']
    dft['vol-hcp'] = hcp_dft['volume']
    dft['energy-hcp'] = hcp_dft['energy']
    dft.to_csv(f'{system}_dft.csv', index=0) 
    return dft, fcc_svn, hcp_svn

def normalize_energies(df1, df2, df3):
    dft_fcc = df1['energy-fcc'].to_numpy()
    dft_hcp = df1['energy-hcp'].to_numpy()
    l3i3_fcc = df2['energy-l3i3'].to_numpy()
    l3i3_hcp = df3['energy-l3i3'].to_numpy()
    l4i3_fcc = df2['energy-l4i3'].to_numpy()
    l4i3_hcp = df3['energy-l4i3'].to_numpy()
    l3i5_fcc = df2['energy-l3i5'].to_numpy()
    l3i5_hcp = df3['energy-l3i5'].to_numpy()
    ...
    df1.to_csv(f'{system}_dft.csv')
    df2.to_csv(f'svn_{system}-fcc.csv')
    df3.to_csv(f'svn_{system}-hcp.csv')
